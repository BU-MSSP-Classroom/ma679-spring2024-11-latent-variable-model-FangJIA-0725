...
library(lavaan)
library(semPlot)
latent_formula1 <- 'CombatPower =~ HP + Attack + Defense + Sp..Atk + Sp..Def + Speed'

latent_model1 <- sem(latent_formula1, data=Pokemon_x_scaled)

summary(latent_model1, standardize = T, rsq = T)
semPaths(latent_model1, whatLabels="est", layout="tree", rotation=2)
```
...
The Confirmatory Factor Analysis (CFA) revealed a single latent variable, Combat Power (CmP), which effectively captures the underlying structure of the combat-related stats in the Pokémon dataset. The factor loadings indicate that Special Attack (Sp. Atk) and Special Defense (Sp. Def) are the most influenced by CmP, as evidenced by their higher loadings and R-square values. Regular Defense also shows a strong relationship with CmP, suggesting that these defense-related attributes are key components of a Pokémon's overall combat efficacy. The lower R-square values for HP and Speed indicate that these stats have additional influences beyond CmP, warranting further investigation. 
~~~
...
evolution_model <- 'evolution =~ Stage_1 + Stage_2 + Stage_3'

model<- sem(evolution_model, data = Pokemon_evo, ordered = names(Pokemon_evo))
...
The statistical results from the latent variable analysis suggest that there are significant differences in the stages of Pokémon evolution and their relationship to the 'evolution'. Specifically, Stage 2 Pokémon exhibit a very high loading (29.207) with a large standard error, indicating potential issues with estimation or model specification. However, Stage 3 has a substantial and statistically significant loading (5.383, p < 0.01), showing a strong relationship with the evolution latent variable. The variance for evolution is very low (0.003), suggesting that the latent variable has little variability not explained by the model. In summary, while Stage 3 Pokémon seem to have a strong association with the evolution construct, the model perform poorly, especially considering the unexpected negative variance estimate for Stage 2, to improve its interpretative value.
~~~
